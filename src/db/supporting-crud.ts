...
export class SupportingCrud {
  ...
  async createActor(p: PrincipalContext, i: Pick<Actor, "type" | "name" | "description" | "metadata">): Promise<Actor> {
    return this.createOwned(p, "actor", `INSERT INTO actor (type,name,description,metadata,created_by_principal_id) VALUES ($1,$2,$3,$4,$5) RETURNING *`, [i.type,i.name,i.description ?? null,i.metadata ?? {}], actor);
  }
  ...
  async updateActor(p:PrincipalContext,id:Id,i:Partial<Pick<Actor,"type"|"name"|"description"|"metadata">>):Promise<Actor>{await requireAccess(this,p,"actor",id,"editor");const r=await this.db.query("UPDATE actor SET type=COALESCE($2,type),name=COALESCE($3,name),description=COALESCE($4,description),metadata=COALESCE($5,metadata),updated_at=now() WHERE id=$1 RETURNING *",[id,i.type??null,i.name??null,i.description??null,i.metadata??null]);if(!r.rows[0])throw new Error("Actor disappeared during update");return actor(r.rows[0]);}
  ...
  async createSchedule(p:PrincipalContext,i:{title:string;startAt:string;endAt?:string;recurrence?:string;timezone?:string;projectId?:Id;goalId?:Id;taskId?:Id}):Promise<Schedule>{return this.createOwned(p,"schedule",`INSERT INTO schedule (project_id,goal_id,task_id,title,start_at,end_at,recurrence,timezone,created_by_principal_id) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *`,[i.projectId??null,i.goalId??null,i.taskId??null,i.title,i.startAt,i.endAt??null,i.recurrence??null,i.timezone??null],schedule);}
  ...
  async createAlarm(p:PrincipalContext,i:{title:string;triggerAt:string;recurrence?:string;timezone?:string;projectId?:Id;goalId?:Id;taskId?:Id}):Promise<Alarm>{return this.createOwned(p,"alarm",`INSERT INTO alarm (project_id,goal_id,task_id,title,trigger_at,recurrence,timezone,created_by_principal_id) VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *`,[i.projectId??null,i.goalId??null,i.taskId??null,i.title,i.triggerAt,i.recurrence??null,i.timezone??null],alarm);}
